...
import type { LeadDetailData, LeadStatus, TimelineEntry } from '@/types/admin';
import { LEAD_TRANSITIONS } from '@/types/admin';
...
function LeadDetail({ data, adminUserId, adminEmail }: LeadDetailProps) {
  const { lead } = data;
  const [timelineEntries, setTimelineEntries] = useState<TimelineEntry[]>(data.timeline);
  const [currentStatus, setCurrentStatus] = useState<LeadStatus>(lead.estado as LeadStatus);
  ...
  const handleStatusSelect = useCallback((newStatus: LeadStatus) => {
  ...
  }, [dialogNewStatus, dialogNotas, currentStatus, lead.id, adminEmail]);

  const handleNoteAdded = useCallback((entry: TimelineEntry) => {
    setTimelineEntries(prev => [entry, ...prev]);
  }, []);

  const availableTransitions = LEAD_TRANSITIONS[currentStatus] ?? [];
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {lead.comuna_nombre && (
              <div className="flex items-start gap-3">
                <MapPinIcon className="mt-0.5 h-4 w-4 text-muted-foreground shrink-0" />
                <div>
                  <p className="text-xs text-muted-foreground">Comuna</p>
                  <p className="text-sm font-medium">{lead.comuna_nombre}</p>
                </div>
              </div>
            )}
            {lead.kit_label && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{lead.kit_label}</p>
                </div>
              </div>
            )}
            <div className="flex items-start gap-3">
              <div className="mt-0.5 h-4 w-4 shrink-0 flex items-center justify-center text-muted-foreground">$</div>
              <div>
                <p className="text-xs text-muted-foreground">Precio Final IVA</p>
                <p className="text-sm font-medium">{formatCLP(lead.precio_final_iva)}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
